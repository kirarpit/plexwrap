...
import './App.css';
import { getUsers, getWrap, getWrapByToken, getTokenForUser, User, WrapData } from './api';
...
function App() {
  ...
  useEffect(() => {
    ...
    const hasImages = wrapData.cards?.some(card => card.generated_image) ?? false;
    
    // Only allow img mode if images are available
    if (modeParam === 'img' && hasImages) {
      setDisplayMode('img');
    } else if (modeParam === 'html' || !hasImages) {
      setDisplayMode('html');
      // If images aren't available but mode was set to img, update URL
      if (modeParam === 'img' && !hasImages) {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      setLoading(false);
      setError('A shareable token is required to access wraps. Please use a valid share link.');
    }
  }, []);
  ...
  const loadWrapByToken = async (token: string) => {
    try {
      ...
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to load wrap. The token may be invalid.';
      console.error('Error loading wrap by token:', err);
      console.error('Token used:', token);
      console.error('API Base URL:', process.env.REACT_APP_API_URL || 'auto-detected');
